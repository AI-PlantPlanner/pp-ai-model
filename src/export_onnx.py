...
ONNX_OUTPUT_PATH = config.MODEL_OUTPUT_PATH_V1.with_suffix(".onnx")


def convert(model_path=config.MODEL_OUTPUT_PATH_V1, onnx_path=ONNX_OUTPUT_PATH) -> tuple:
...
def verify(onnx_path, model, feature_columns, n_samples=2000) -> dict:
    ...
    X = build_features_v1(df).reindex(columns=feature_columns, fill_value=0)
...
def main() -> None:
    ...
    joblib_size_mb = os.path.getsize(config.MODEL_OUTPUT_PATH_V1) / 1e6
